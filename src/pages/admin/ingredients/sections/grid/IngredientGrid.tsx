...
const IngredientGrid = () => {
        ...
        const columns: GridColDef[] = [
          ...
          },
          {
            ...
            cellClassName: (params) => {
              ...
              if (row.isEdit && !value || validateDuplicateName(value, id))
                return "error-cell"
              return ""
            },
            flex: 1,
  ...
  const onProcessRowUpdate = (newRow: IngredientDataSource, oldRow: IngredientDataSource) => {
    ...
    Object.keys(cloneRow).forEach(key => {
      if (typeof cloneRow[key] === 'string')
        cloneRow[key] = cloneRow[key].trim()
    })
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          ...
          <Stack direction="row" alignItems="center" gap={2}>
            ...
            <Tooltip title="Thêm dòng">
              <FontAwesomeIcon onClick={(e: any) => handleAddBlankRow()} cursor="pointer" fontSize="24px" color={theme.palette.primary.main} icon={faPlusCircle} />
            </Tooltip>
          </Stack>
        </Stack>
        <Stack gap={2}>
          ...
          />
          <Button variant='contained' sx={{ alignSelf: 'flex-end' }} onClick={(e: any) => handleSubmitData()}>Lưu dữ liệu</Button>
          <Typography variant='body2' color="text.secondary">*Lưu ý: Những ô dữ liệu báo đỏ khi chưa nhập đủ dữ liệu hoặc sai định dạng dữ liệu.</Typography>
        </Stack>
      </Wrapper>
      {
        listGridErrors.length > 0 &&
        <ModalAlertValidate listErrors={listGridErrors} />
      }
    </ShadowBox>
  );
};
